...
//   반동 오프셋 갱신(recoil) → 그 위에 퍼짐 샘플(spread) → 레이캐스트 → 탄자국   (시뮬레이터 tryFire 순서)
// 탄약·재장전·질주 중 사격 불가는 T12, 표적 판정·피해는 T13, ADS 배율은 T11.
export function createShooter(weapon, recoil, spread, mouseButtons, deps) {
  ...
  function fireOne(nowMs) {
    ...
    const sp = spread.current(1, moveMul());
  ...
  function update(nowMs, dtSec) {
    ...
    state.currentSpread = spread.current(1, moveMul());
  }
