...
export const config = {
  ...
  monster: {
    aggroRange: 40,             // 인지 거리 (m). 시야가 있어야 함. 맞으면 즉시 인지 (T55 사용자 결정 2026-09-07: 30→40 — 구역 2 입구 끝(28 m)까지 닿게)
    laserDelay: 0.5,            // T55: 조준 시작 뒤 조준선이 보이기까지 (s). 원거리형·보스 공통. 발사 시점(aimDelay)은 그대로 — aimDelay ≤ 이 값이면 조준선 없이 쏜다
    radius: 0.4,                // 충돌 반지름 (m)
    melee: {
    ...
    ranged: {
      ...
      fireRange: 40,            // 이 거리 안 + 시야 있으면 조준 시작 (T55 사용자 결정: 25→40)
